...
class Form extends Component {
 ...
 componentWillReceiveProps(nextProps) {
  ...
  render () {
    return (
      <div className="container" style={{  width: '700px',backgroundColor:'white'}}>
      <form className="demoForm form-group" onSubmit={this.ok}>
        <h2>Sign up</h2>
        <div className={`form-group`}>
          <label htmlFor="email">Email address :</label>
          <input type="email" required className="form-control" name="Adresse" autoComplete="off"
            placeholder="Email"
        ...
        <input type="submit" className="btn btn-primary" value="Ajouter" disabled={!this.state.formValid} />
      </form>
      </div>
    )
  }
